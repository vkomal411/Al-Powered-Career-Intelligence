...
import CareerSuggestionPage from "../../components/CareerSuggestion/CareerSuggestionPage";
import { apiFetch, UserResponse, logoutUser } from "../../lib/api";

export default function CareerSuggestionRoute() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-paper dark:bg-[#090d16]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Loading Career Suggestion Engine...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex min-h-screen items-center justify-center bg-paper dark:bg-[#090d16] px-6">
        <div className="w-full max-w-md rounded-2xl border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-[#111726] p-8 text-center shadow-card">
          ...
          <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
            Please sign in to access personalized Career Suggestions.
          </p>
          <button
            type="button"
            onClick={() => router.push("/login?redirect=/career-tools/career-suggestion")}
  ...
  return (
    <>
      <Head>
        <title>Career Suggestion — CareerPilot.AI</title>
        <meta
          name="description"
          content="AI-powered career suggestions matched deterministically against your resume skills and market benchmarks."
        />
      </Head>
      <div className="min-h-screen bg-paper dark:bg-[#090d16]">
        <Topbar fullName={user.full_name} onLogout={handleLogout} activeMenu="career-suggestion" />

        <main className="mx-auto max-w-7xl px-6 sm:px-8 py-8 space-y-6">
          <GroupNavControl group="career-tools" activeId="career-suggestion" />
