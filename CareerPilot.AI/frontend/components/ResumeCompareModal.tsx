...
export default function ResumeCompareModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-xs animate-fade-in">
      <div className="relative w-full max-w-4xl rounded-2xl bg-white dark:bg-[#111726] p-6 shadow-2xl dark:shadow-[0_16px_48px_rgba(0,0,0,0.7)] border border-slate-200 dark:border-white/[0.1] space-y-6 max-h-[90vh] flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-white/[0.08] pb-4">
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-xl bg-indigo-50 text-indigo-600 border border-indigo-100 dark:bg-indigo-500/15 dark:text-indigo-300 dark:border-indigo-500/30">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Resume 1 Selector */}
          <div className="p-4 rounded-xl border border-indigo-200 dark:border-indigo-500/30 bg-indigo-50/40 dark:bg-indigo-500/10 space-y-2">
            <label className="text-[10px] font-bold text-indigo-800 dark:text-indigo-300 uppercase tracking-wider block">
              Primary Resume (Version A)
            </label>
            <select
              value={resume1Id}
              onChange={(e) => setResume1Id(e.target.value)}
              className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-xs font-semibold text-slate-900 dark:text-white focus:outline-none focus:border-indigo-600 dark:focus:border-indigo-500"
            >
              {resumes.map((r) => (
            ...
            {r1 && (
              <div className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400 pt-1">
                ...
                <span className="font-bold text-indigo-700 dark:text-indigo-400">{r1.extracted_skills?.length || 0} Skills</span>
              </div>
            )}
          </div>
          ...
          <div className="p-4 rounded-xl border border-indigo-200 dark:border-indigo-500/30 bg-indigo-50/40 dark:bg-indigo-500/10 space-y-2">
            ...
            {r2 && (
              <div className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400 pt-1">
                ...
                <span className="font-bold text-indigo-700 dark:text-indigo-400">{r2.extracted_skills?.length || 0} Skills</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto rounded-xl border border-slate-200 dark:border-white/[0.08] bg-slate-50/50 dark:bg-slate-800/40 p-4 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-xl bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-white/[0.06] shadow-2xs space-y-3">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-white/[0.06] pb-2">
                ...
                <span className="text-xs font-extrabold text-indigo-600 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-500/20 px-2 py-0.5 rounded">
            ...
            <div className="p-4 rounded-xl bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-white/[0.06] shadow-2xs space-y-3">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-white/[0.06] pb-2">
                ...
                <span className="text-xs font-extrabold text-purple-600 dark:text-purple-300 bg-purple-50 dark:bg-purple-500/20 px-2 py-0.5 rounded">
