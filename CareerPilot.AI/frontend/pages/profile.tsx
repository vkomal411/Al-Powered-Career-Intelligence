          ...
          <div className="grid grid-cols-1 md:grid-cols-[250px_1fr] gap-8 items-start">
              ...
              <button
                ...
                className={`group flex items-center gap-3 w-full px-4 py-3 text-sm font-semibold rounded-xl transition-all duration-200 ease-in-out active:scale-[0.99] ${
                  activeTab === "personal"
                    ...
                    : "text-slate-500 hover:bg-slate-50 hover:text-ink hover:translate-x-1"
                }`}
              >
            ...
            <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
              <div className="border-b border-slate-100 px-6 py-5 bg-slate-50/50">
                <h3 className="font-display font-bold text-base text-ink capitalize">
              ...
              <form onSubmit={handleUpdateProfile} className="p-6 space-y-6">
                {/* Alerts */}
                {error && (
                  <div className="rounded-xl border border-red-100 bg-red-50/40 p-4 text-sm font-medium text-danger">
                    {error}
                  </div>
                )}
                {success && (
                  <div className="rounded-xl border border-emerald-100 bg-emerald-50/40 p-4 text-sm font-medium text-verified">
                    {success}
                  </div>
                )}
                  ...
                  <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl space-y-5">
                      <div className="flex items-center gap-3 border-b border-slate-100 pb-3">
                        ...
                        <div>
                          <h3 className="text-base font-bold text-ink">Confirm Profile Sync</h3>
                          <p className="text-xs text-slate-500">Review new skills detected from your resume</p>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto p-3 rounded-xl border border-slate-150 bg-slate-50">
                          {pendingSync.addedSkills.map((sk, i) => (
                            <span key={i} className="inline-flex items-center gap-1 rounded-lg bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700 border border-emerald-200">
                              + {sk}
                            </span>
                          ))}
                        </div>
                ...
                {/* Submit button */}
                <div className="flex items-center justify-end border-t border-slate-100 pt-5 mt-5">
                  <button
                    type="submit"
                    disabled={saving}
