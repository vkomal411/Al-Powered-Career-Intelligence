...
export default function ResumeBuilderPage() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-paper dark:bg-[#090d16]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Loading Resume Studio v2.0...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-paper dark:bg-[#090d16] px-6">
        <div className="w-full max-w-md rounded-2xl border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-[#111726] p-8 text-center shadow-card">
          ...
          <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">Please sign in to access Resume Studio v2.0.</p>
          <button
            type="button"
            onClick={() => router.push("/login?redirect=/resume-tools/resume-builder")}
  ...
  return (
    <>
      <Head>
        <title>CareerPilot AI Resume Studio v2.0 — Resume Tools</title>
      </Head>
      <div className="min-h-screen bg-paper dark:bg-[#090d16]">
        ...
        <main className="mx-auto max-w-7xl px-6 sm:px-8 py-8 space-y-6">
          <GroupNavControl group="resume-tools" activeId="resume-builder" />
          <ResumeEditorStudio />
        </main>
      </div>
    </>
